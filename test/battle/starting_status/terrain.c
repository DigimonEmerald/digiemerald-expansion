...
SINGLE_BATTLE_TEST("Terrain started after the one which started the battle lasts only 5 turns")
{
    ...
    GIVEN {
<<<<<<< HEAD:test/battle/terrain/starting_terrain.c
        PLAYER(SPECIES_LOPMONX) { Ability(viaMove == TRUE ? ABILITY_SHADOW_TAG : ABILITY_GRASSY_SURGE); }
        OPPONENT(SPECIES_LOPMONX);
=======
...
>>>>>>> upstream/master:test/battle/starting_status/terrain.c
        ...
        if (viaMove) {
<<<<<<< HEAD:test/battle/terrain/starting_terrain.c
            MESSAGE("Lopmonx used GrssyTerrain!");
=======
