...
DOUBLE_BATTLE_TEST("Defrost: A fire type spread move will thaw both targets")
{
    GIVEN {
        ...
        PLAYER(SPECIES_LOPMON_X);
        OPPONENT(SPECIES_LOPMON_X) { Status1(STATUS1_FREEZE); }
        OPPONENT(SPECIES_LOPMON_X) { Status1(STATUS1_FREEZE); }
    } WHEN {
...
{
    GIVEN {
        PLAYER(SPECIES_LOPMON_X) { Status1(STATUS1_FROSTBITE); }
        OPPONENT(SPECIES_LOPMON_X) { Status1(STATUS1_SLEEP_TURN(3)); }
    } WHEN {
