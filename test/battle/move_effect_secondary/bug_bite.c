...
SINGLE_BATTLE_TEST("Bug Bite eats the target's berry and immediately gains its effect")
{
    ...
    GIVEN {
        PLAYER(SPECIES_LOPMONX) { HP(399); MaxHP(400); Status1(status1); Moves(MOVE_SLEEP_TALK, MOVE_BUG_BITE); }
        ...
        if (effect == HOLD_EFFECT_RESTORE_HP || effect == HOLD_EFFECT_ENIGMA_BERRY) {
            ...
            if (statId == STAT_ATK) {
                ...
                if (item == ITEM_APICOT_BERRY) {
                    ...
                    MESSAGE("Using Maranga Berry, the Sp. Def of Lopmonx rose!");
                }
...
SINGLE_BATTLE_TEST("Tanga Berry activates before Bug Bite")
{
    GIVEN {
        ...
        OPPONENT(SPECIES_LOPMONX) {Item(ITEM_TANGA_BERRY); }
    } WHEN {
        TURN { MOVE(player, MOVE_BUG_BITE); }
    } SCENE {
        MESSAGE("Lopmonx used Bug Bite!");
