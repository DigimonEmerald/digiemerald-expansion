...
WILD_BATTLE_TEST("Pokemon gain exp after catching a Pokemon")
{
    ...
    GIVEN {
        PLAYER(SPECIES_LOPMONX) { Level(level); }
...
WILD_BATTLE_TEST("Higher leveled Pokemon give more exp", s32 exp)
{
    ...
    GIVEN {
        PLAYER(SPECIES_LOPMONX) { Level(20); }
        OPPONENT(SPECIES_DATIRIMON) { Level(level); HP(1); }
    } WHEN {
...
WILD_BATTLE_TEST("Exp is scaled to player and opponent's levels", s32 exp)
{
    ...
    GIVEN {
        PLAYER(SPECIES_LOPMONX) { Level(level); }
        OPPONENT(SPECIES_DATIRIMON) { Level(5); HP(1); }
    } WHEN {
        TURN { MOVE(player, MOVE_TACKLE); }
    } SCENE {
<<<<<<< HEAD
...
WILD_BATTLE_TEST("Exp Share(held) gives Experience to mons which did not participate in battle")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_EXVEEMON) { Level(40); Item(item); }
        OPPONENT(SPECIES_DATIRIMON) { Level(10); HP(1); }
    } WHEN {
        TURN { MOVE(player, MOVE_TACKLE); }
    } SCENE {
...
>>>>>>> upstream/master
        ...
        if (item == ITEM_EXP_SHARE)
            EXPECT_GT(GetMonData(&gPlayerParty[1], MON_DATA_EXP), gExperienceTables[gSpeciesInfo[SPECIES_EXVEEMON].growthRate][40]);
        else
            EXPECT_EQ(GetMonData(&gPlayerParty[1], MON_DATA_EXP), gExperienceTables[gSpeciesInfo[SPECIES_EXVEEMON].growthRate][40]);
    }
}
