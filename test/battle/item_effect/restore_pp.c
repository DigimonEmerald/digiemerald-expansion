...
SINGLE_BATTLE_TEST("Ether restores the PP of one of a battler's moves by 10 ")
{
    GIVEN {
        ...
        PLAYER(SPECIES_LOPMONX) { MovesWithPP({MOVE_TACKLE, 0}, {MOVE_CONFUSION, 20}); }
...
SINGLE_BATTLE_TEST("Elixir restores the PP of all of a battler's moves by 10")
{
    GIVEN {
        ...
        PLAYER(SPECIES_LOPMONX) { MovesWithPP({MOVE_TACKLE, 0}, {MOVE_CONFUSION, 0}, {MOVE_SCRATCH, 0}, {MOVE_GROWL, 0}); }
        OPPONENT(SPECIES_LOPMONX);
    } WHEN {
