...
SINGLE_BATTLE_TEST("Spicy Extract raises target's Attack by 2 stages and lowers target's Defense by 2 stages")
{
    GIVEN {
        ...
        MESSAGE("The opposing Lopmonx's Attack sharply rose!");
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_STATS_CHANGE, opponent);
...
SINGLE_BATTLE_TEST("Spicy Extract Defense loss is prevented by Big Pecks")
{
    GIVEN {
        ...
        ANIMATION(ANIM_TYPE_MOVE, MOVE_SPICY_EXTRACT, player);
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_STATS_CHANGE, opponent);
        MESSAGE("The opposing Ketomon's Attack sharply rose!");
        NONE_OF {
            ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_STATS_CHANGE, opponent);
            MESSAGE("The opposing Lopmonx's Defense harshly fell!");
        }
...
SINGLE_BATTLE_TEST("Spicy Extract will fail if target is in a semi-invulnerability state")
{
    GIVEN {
        ...
        MESSAGE("The opposing Lopmonx used Dive!");
...
SINGLE_BATTLE_TEST("Spicy Extract stat changes will be inverted by Contrary")
{
    GIVEN {
        ...
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_STATS_CHANGE, opponent);
        MESSAGE("The opposing D_devimon's Attack harshly fell!");

        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_STATS_CHANGE, opponent);
        MESSAGE("The opposing D_devimon's Defense sharply rose!");
    } THEN {
        EXPECT_EQ(opponent->statStages[STAT_ATK], DEFAULT_STAT_STAGE - 2);
        EXPECT_EQ(opponent->statStages[STAT_DEF], DEFAULT_STAT_STAGE + 2);
    }
...
SINGLE_BATTLE_TEST("Spicy Extract against Clear Amulet and Contrary raises Defense only")
{
    GIVEN {
        PLAYER(SPECIES_LOPMONX);
        OPPONENT(SPECIES_D_DEVIMON) { Ability(ABILITY_CONTRARY); Item(ITEM_CLEAR_AMULET); }
    } WHEN {
        TURN { MOVE(player, MOVE_SPICY_EXTRACT); }
    } SCENE {
        MESSAGE("Lopmonx used Spicy Extract!");
        ...
        MESSAGE("The effects of the Clear Amulet held by the opposing D_devimon prevents its stats from being lowered!");
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_STATS_CHANGE, opponent);
        MESSAGE("The opposing D_devimon's Defense sharply rose!");
    } THEN {
...
AI_DOUBLE_BATTLE_TEST("Spicy Extract user will use it if partner holds Clear Amulet and a physical move")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_LOPMONX) { Speed(10); }
        OPPONENT(SPECIES_LOPMONX) { Speed(20); Item(ITEM_CLEAR_AMULET); Moves(move); }
        OPPONENT(SPECIES_LOPMONX) { Speed(40); Moves(MOVE_TACKLE, MOVE_SPICY_EXTRACT); }
    } WHEN {
        TURN {
            if (move == MOVE_TACKLE)
...
AI_DOUBLE_BATTLE_TEST("Spicy Extract user will not choose the move if it does not benefit partner")
{
    ...
    PARAMETRIZE { species = SPECIES_D_DEVIMON; ability = ABILITY_CONTRARY; }
