...
SINGLE_BATTLE_TEST("Razor Wind needs a charging turn")
{
    GIVEN {
        ...
        if (B_UPDATED_MOVE_DATA >= GEN_5) {
            NOT MESSAGE("Lopmonx whipped up a whirlwind!");
            MESSAGE("Lopmonx used Razor Wind!");
        } else {
            ANIMATION(ANIM_TYPE_MOVE, MOVE_RAZOR_WIND, player);
        }
        if (B_UPDATED_MOVE_DATA < GEN_5)
            MESSAGE("Lopmonx whipped up a whirlwind!");
        else
            ANIMATION(ANIM_TYPE_MOVE, MOVE_RAZOR_WIND, player);
        // Attack turn
        MESSAGE("Lopmonx used Razor Wind!");
...
SINGLE_BATTLE_TEST("Skull Bash needs a charging turn")
{
    GIVEN {
        ...
        // Charging turn
        if (B_UPDATED_MOVE_DATA >= GEN_5) {
            NOT MESSAGE("Lopmonx lowered its head!");
            MESSAGE("Lopmonx used Skull Bash!");
        } else
            ANIMATION(ANIM_TYPE_MOVE, MOVE_SKULL_BASH, player);
        if (B_UPDATED_MOVE_DATA < GEN_5)
            MESSAGE("Lopmonx lowered its head!");
        else
            ANIMATION(ANIM_TYPE_MOVE, MOVE_SKULL_BASH, player);
...
SINGLE_BATTLE_TEST("Sky Attack needs a charging turn")
{
    GIVEN {
        ...
        if (B_UPDATED_MOVE_DATA >= GEN_5) {
            NONE_OF {
                MESSAGE("Lopmonx became cloaked in a harsh light!");
                MESSAGE("Lopmonx is glowing!");
            }
            MESSAGE("Lopmonx used Sky Attack!");
        } else
...
SINGLE_BATTLE_TEST("Electro Shot needs a charging Turn")
{
    GIVEN {
        ...
        // Charging turn
        MESSAGE("Lopmonx used Electro Shot!");
        ANIMATION(ANIM_TYPE_MOVE, MOVE_ELECTRO_SHOT, player);
        MESSAGE("Lopmonx absorbed electricity!");
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_STATS_CHANGE, player);
