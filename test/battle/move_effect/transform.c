...
SINGLE_BATTLE_TEST("(TERA) Transform does not copy the target's Tera Type, and if the user is Terastallized it keeps its own Tera Type")
{
    ...
    GIVEN {
        PLAYER(SPECIES_LOPMON_X) { Moves(MOVE_CELEBRATE, MOVE_SCRATCH, MOVE_EARTHQUAKE); TeraType(TYPE_GHOST); }
        ...
        // turn 3
        MESSAGE("Lopmon_x used Earthquake!");
        MESSAGE("It doesn't affect the opposing Crabmon…");
        NOT { HP_BAR(opponent); }
    }
}
