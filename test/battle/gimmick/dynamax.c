...
SINGLE_BATTLE_TEST("Dynamax: Dynamaxed Pokemon are affected by Grudge")
{
    GIVEN {
        ...
        MESSAGE("Lopmonx used Max Strike!");
        MESSAGE("Lopmonx's Scratch lost all its PP due to the grudge!");
...
SINGLE_BATTLE_TEST("Dynamax: Dynamaxed Pokemon can have base moves disabled on their first turn")
{
    GIVEN {
        ...
        MESSAGE("Lopmonx used Scratch!");
        MESSAGE("The opposing Lopmonx used Disable!");
        MESSAGE("Lopmonx's Scratch was disabled!");
        MESSAGE("Lopmonx used Max Strike!");
    }
}
...
{
    GIVEN {
        ASSUME(GetMoveCategory(MOVE_DRAGON_CLAW) == DAMAGE_CATEGORY_PHYSICAL); // Otherwise Blgargomon faints.
        ...
        OPPONENT(SPECIES_BLGARGOMON) { Ability(ABILITY_PRANKSTER); Moves(MOVE_CELEBRATE); }
        OPPONENT(SPECIES_WYNAUT);
    } WHEN {
