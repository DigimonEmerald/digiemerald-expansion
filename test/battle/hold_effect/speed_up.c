...
SINGLE_BATTLE_TEST("Salac Berry raises the holder's Speed by one stage when HP drops to 1/4 or below")
{
    ...
    GIVEN {
        PLAYER(SPECIES_LOPMONX) { MaxHP(160); HP(80); Item(ITEM_SALAC_BERRY); }
        ...
        if (move == MOVE_SCRATCH) {
            NONE_OF {
                ...
                MESSAGE("Using Salac Berry, the Speed of Lopmonx rose!");
            }
        } else {
            ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_HELD_ITEM_EFFECT, player);
            MESSAGE("Using Salac Berry, the Speed of Lopmonx rose!");
...
{
    GIVEN {
        PLAYER(SPECIES_KYAROMON) { MaxHP(80); HP(80); Ability(ABILITY_GLUTTONY); Item(ITEM_SALAC_BERRY); }
        OPPONENT(SPECIES_LOPMONX);
    } WHEN {
        ...
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_HELD_ITEM_EFFECT, player);
        MESSAGE("Using Salac Berry, the Speed of Kyaromon rose!");
    } THEN {
        EXPECT_EQ(player->statStages[STAT_SPEED], DEFAULT_STAT_STAGE + 1);
    }
}
...
DOUBLE_BATTLE_TEST("Salac Berry does not miss timing miss timing")
{
    GIVEN {
        ...
        PLAYER(SPECIES_LOPMONX);
        OPPONENT(SPECIES_EXVEEMON) { MaxHP(100); HP(26); Item(ITEM_SALAC_BERRY); }
        OPPONENT(SPECIES_LOPMONX);
    } WHEN {
        ...
        MESSAGE("Using Salac Berry, the Speed of the opposing Exveemon rose!");
