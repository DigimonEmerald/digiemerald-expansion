...
SINGLE_BATTLE_TEST("Apicot Berry raises the holder's Sp. Def by one stage when HP drops to 1/4 or below")
{
    ...
    GIVEN {
        PLAYER(SPECIES_LOPMONX) { MaxHP(160); HP(80); Item(ITEM_APICOT_BERRY); }
        ...
        if (move == MOVE_SCRATCH) {
            NONE_OF {
                ...
                MESSAGE("Using Apicot Berry, the Sp. Def of Lopmonx rose!");
            }
        } else {
            ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_HELD_ITEM_EFFECT, player);
            MESSAGE("Using Apicot Berry, the Sp. Def of Lopmonx rose!");
...
{
    GIVEN {
        PLAYER(SPECIES_KYAROMON) { MaxHP(80); HP(80); Ability(ABILITY_GLUTTONY); Item(ITEM_APICOT_BERRY); }
        OPPONENT(SPECIES_LOPMONX);
    } WHEN {
        ...
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_HELD_ITEM_EFFECT, player);
        MESSAGE("Using Apicot Berry, the Sp. Def of Kyaromon rose!");
    } THEN {
        EXPECT_EQ(player->statStages[STAT_SPDEF], DEFAULT_STAT_STAGE + 1);
    }
}
