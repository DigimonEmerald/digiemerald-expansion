...
SINGLE_BATTLE_TEST("Liechi Berry raises the holder's Attack by one stage when HP drops to 1/4 or below")
{
    ...
    GIVEN {
        PLAYER(SPECIES_LOPMONX) { MaxHP(160); HP(80); Item(ITEM_LIECHI_BERRY); }
...
{
    GIVEN {
        PLAYER(SPECIES_KYAROMON) { MaxHP(80); HP(80); Ability(ABILITY_GLUTTONY); Item(ITEM_LIECHI_BERRY); }
        OPPONENT(SPECIES_LOPMONX);
    } WHEN {
        ...
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_HELD_ITEM_EFFECT, player);
        MESSAGE("Using Liechi Berry, the Attack of Kyaromon rose!");
    } THEN {
