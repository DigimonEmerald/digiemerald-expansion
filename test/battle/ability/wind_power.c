...
SINGLE_BATTLE_TEST("Wind Power sets up Charge for opponent when hit by a wind move")
{
    ...
    GIVEN {
        PLAYER(SPECIES_LOPMONX) {Ability(ABILITY_LIMBER); Speed(5) ;} // Limber, so it doesn't get paralyzed.
...
DOUBLE_BATTLE_TEST("Wind Power activates correctly for every battler with the ability when hit by a 2/3 target move")
{
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_LOPMONX) { Ability(ABILITY_LIMBER); Speed(20); }
        OPPONENT(SPECIES_LOPMONX) { Ability(ABILITY_LIMBER); Speed(15); }
    } WHEN {
