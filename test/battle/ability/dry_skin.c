...
{
    GIVEN {
        PLAYER(SPECIES_BABYMON) { Ability(ABILITY_DRY_SKIN); HP(100); MaxHP(200); }
        ...
        MESSAGE("Babymon's Dry Skin takes its toll!");
    }
}
...
SINGLE_BATTLE_TEST("Dry Skin heals 1/8th Max HP in Rain")
{
    GIVEN {
        ...
        ABILITY_POPUP(player, ABILITY_DRY_SKIN);
        MESSAGE("Babymon's Dry Skin restored its HP a little!");
        HP_BAR(player, damage: -(200 / 8));
    }
}
...
SINGLE_BATTLE_TEST("Dry Skin increases damage taken from Fire-type moves by 25%", s16 damage)
{
    ...
    GIVEN {
        ...
        ASSUME(gSpeciesInfo[SPECIES_BABYMON].types[0] == TYPE_BUG);
        ASSUME(gSpeciesInfo[SPECIES_BABYMON].types[1] == TYPE_GRASS);
        ASSUME(gSpeciesInfo[SPECIES_WOBBUFFET].types[0] == TYPE_PSYCHIC);
        ASSUME(gSpeciesInfo[SPECIES_WOBBUFFET].types[1] == TYPE_PSYCHIC);
        PLAYER(SPECIES_WOBBUFFET) { SpAttack(71); }
        OPPONENT(SPECIES_BABYMON) { Ability(ability); SpDefense(165); }
    } WHEN {
...
SINGLE_BATTLE_TEST("Dry Skin heals 25% when hit by water type moves")
{
    GIVEN {
        ...
        MESSAGE("Babymon restored HP using its Dry Skin!");
    }
}
...
SINGLE_BATTLE_TEST("Dry Skin does not activate if protected")
{
    GIVEN {
        ...
        NONE_OF { ABILITY_POPUP(player, ABILITY_DRY_SKIN); HP_BAR(player); MESSAGE("Babymon restored HP using its Dry Skin!"); }
    }
}
...
SINGLE_BATTLE_TEST("Dry Skin prevents Absorb Bulb and Luminous Moss from activating")
{
    ...
    GIVEN {
        ASSUME(gMovesInfo[MOVE_BUBBLE].type == TYPE_WATER);
        PLAYER(SPECIES_BABYMON) { Ability(ABILITY_DRY_SKIN); HP(100); MaxHP(200); Item(item); }
        OPPONENT(SPECIES_WOBBUFFET);
    } WHEN {
        TURN { MOVE(opponent, MOVE_BUBBLE); }
    } SCENE {
