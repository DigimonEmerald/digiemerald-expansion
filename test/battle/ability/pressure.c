...
{
    GIVEN {
        PLAYER(SPECIES_LOPMONX) { MovesWithPP({MOVE_POUND, 35}); }
...
{
    GIVEN {
        PLAYER(SPECIES_LOPMONX) { MovesWithPP({MOVE_IMPRISON, 10}, {MOVE_SNATCH, 10}); }
        OPPONENT(SPECIES_LOPMONX) { Ability(ABILITY_PRESSURE); }
    } WHEN {
...
{
    GIVEN {
        PLAYER(SPECIES_LOPMONX) { MovesWithPP({MOVE_SPIKES, 20}, {MOVE_STEALTH_ROCK, 20}, {MOVE_TOXIC_SPIKES, 20}); }
        OPPONENT(SPECIES_LOPMONX) { Ability(ABILITY_PRESSURE); }
    } WHEN {
...
{
    GIVEN {
        PLAYER(SPECIES_LOPMONX) { MovesWithPP({MOVE_STICKY_WEB, 20}); }
        OPPONENT(SPECIES_LOPMONX) { Ability(ABILITY_PRESSURE); }
    } WHEN {
