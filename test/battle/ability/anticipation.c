...
SINGLE_BATTLE_TEST("Anticipation causes notifies if an opponent has a super-effective move")
{
    GIVEN {
        ...
        OPPONENT(SPECIES_LOPMONX) { Moves(MOVE_CLOSE_COMBAT, MOVE_SCRATCH, MOVE_POUND, MOVE_CELEBRATE); }
    } WHEN {
        TURN { }
    } SCENE {
...
SINGLE_BATTLE_TEST("Anticipation causes notifies if an opponent has a One-hit KO move")
{
    GIVEN {
        ...
        PLAYER(SPECIES_EEVEE) { Ability(ABILITY_ANTICIPATION); }
        OPPONENT(SPECIES_LOPMONX) { Moves(MOVE_FISSURE, MOVE_SCRATCH, MOVE_POUND, MOVE_CELEBRATE); }
    } WHEN {
        TURN { }
    } SCENE {
        ABILITY_POPUP(player, ABILITY_ANTICIPATION);
...
SINGLE_BATTLE_TEST("Anticipation treats Self-Destruct and Explosion like all other Normal types (Gen5+)")
{
    GIVEN {
        ...
        PLAYER(SPECIES_EEVEE) { Ability(ABILITY_ANTICIPATION); }
        OPPONENT(SPECIES_LOPMONX) { Moves(MOVE_EXPLOSION, MOVE_SCRATCH, MOVE_POUND, MOVE_CELEBRATE); }
    } WHEN {
        TURN { }
    } SCENE {
        NOT ABILITY_POPUP(player, ABILITY_ANTICIPATION);
...
SINGLE_BATTLE_TEST("Anticipation treats dynamic move types as their base type (Normal), Natural Gift")
{
    GIVEN {
        ...
        ASSUME(GetSpeciesType(SPECIES_EEVEE, 1) == TYPE_NORMAL);
        PLAYER(SPECIES_EEVEE) { Ability(ABILITY_ANTICIPATION); }
        OPPONENT(SPECIES_LOPMONX) { Item(ITEM_LEPPA_BERRY); Moves(MOVE_NATURAL_GIFT, MOVE_SCRATCH, MOVE_POUND, MOVE_CELEBRATE); }
    } WHEN {
        TURN { }
    } SCENE {
        NOT ABILITY_POPUP(player, ABILITY_ANTICIPATION);
...
{
    GIVEN {
        ASSUME(GetSpeciesType(SPECIES_GEOGREYMON_MEGA, 0) == TYPE_DRAGON);
        ASSUME(GetSpeciesType(SPECIES_GEOGREYMON_MEGA, 1) == TYPE_FLYING);
...
SINGLE_BATTLE_TEST("Anticipation treats Hidden Power as its dynamic type (Gen6+)")
{
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_LOPMONX) { Moves(MOVE_HIDDEN_POWER, MOVE_SCRATCH, MOVE_POUND, MOVE_CELEBRATE); HPIV(30); AttackIV(2); DefenseIV(31); SpAttackIV(30); SpDefenseIV(30); SpeedIV(30); }
    } WHEN {
...
SINGLE_BATTLE_TEST("Anticipation considers Inverse Battle types")
{
    GIVEN {
        ...
        ASSUME(GetSpeciesType(SPECIES_FERROTHORN, 1) == TYPE_STEEL);
        PLAYER(SPECIES_FERROTHORN) { Ability(ABILITY_ANTICIPATION); }
        OPPONENT(SPECIES_LOPMONX) { Moves(MOVE_GROWL, MOVE_SCRATCH, MOVE_POUND, MOVE_CELEBRATE); }
    } WHEN {
        TURN { }
    } SCENE {
        ABILITY_POPUP(player, ABILITY_ANTICIPATION);
