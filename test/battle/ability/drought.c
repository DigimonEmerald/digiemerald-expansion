...
SINGLE_BATTLE_TEST("Drought sets up sun for 5 turns (Gen6+)")
{
    GIVEN {
        ...
        PLAYER(SPECIES_SAKUMON) { Moves(MOVE_CELEBRATE); Ability(ABILITY_DROUGHT); }
        OPPONENT(SPECIES_LOPMON_X) { Moves(MOVE_CELEBRATE); }
    } WHEN {
...
SINGLE_BATTLE_TEST("Drought sets up sun for 8 turns with Heat Rock (Gen6+)")
{
    GIVEN {
        WITH_CONFIG(GEN_CONFIG_ABILITY_WEATHER, GEN_6);
        PLAYER(SPECIES_SAKUMON) { Moves(MOVE_CELEBRATE); Ability(ABILITY_DROUGHT); Item(ITEM_HEAT_ROCK); }
        OPPONENT(SPECIES_LOPMON_X) { Moves(MOVE_CELEBRATE); }
    } WHEN {
        TURN { MOVE(player, MOVE_CELEBRATE); MOVE(opponent, MOVE_CELEBRATE); }
        TURN { MOVE(player, MOVE_CELEBRATE); MOVE(opponent, MOVE_CELEBRATE); }
