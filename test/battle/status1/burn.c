...
SINGLE_BATTLE_TEST("Burn reduces Attack by 50%", s16 damage)
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_LOPMONX) { if (burned) Status1(STATUS1_BURN); }
        OPPONENT(SPECIES_LOPMONX);
    } WHEN {
...
AI_SINGLE_BATTLE_TEST("AI avoids Will-o-Wisp when it can not burn target")
{
    ...
    PARAMETRIZE { species = SPECIES_BOTAMON; ability = ABILITY_BLAZE; }

    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_OMNISCIENT);
        PLAYER(species) { Ability(ability); }
        OPPONENT(SPECIES_LOPMONX) { Moves(MOVE_CELEBRATE, MOVE_WILL_O_WISP); }
    } WHEN {
