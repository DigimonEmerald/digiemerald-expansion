...
AI_SINGLE_BATTLE_TEST("AI_FLAG_PREDICT_MOVE: AI will predict player's move")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_CUTEMON) { Ability(ABILITY_WATER_ABSORB); Moves(MOVE_SURF, MOVE_TACKLE); }
        OPPONENT(SPECIES_CHRYSALMON) { Moves(MOVE_TACKLE); }
        OPPONENT(SPECIES_CUTEMON) { Ability(ABILITY_WATER_ABSORB); Moves(MOVE_TACKLE); }
    } WHEN {
...
AI_SINGLE_BATTLE_TEST("AI_FLAG_PREDICT_MOVE: AI will still attack you when it should")
{
    ...
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_TRY_TO_FAINT | AI_FLAG_CHECK_VIABILITY | AI_FLAG_OMNISCIENT | AI_FLAG_SMART_SWITCHING | AI_FLAG_SMART_MON_CHOICES | AI_FLAG_PREDICT_MOVE);
        PLAYER(SPECIES_CUTEMON) { Ability(ABILITY_WATER_ABSORB); Moves(MOVE_SURF, MOVE_TACKLE); }
        OPPONENT(SPECIES_SYAKOMON) { Moves(MOVE_LEAF_BLADE); }
        OPPONENT(SPECIES_BAKOMON) { Ability(ABILITY_ROCK_HEAD); Moves(MOVE_EARTHQUAKE); }
    } WHEN {
...
AI_SINGLE_BATTLE_TEST("AI won't use Sucker Punch if it expects a move of the same priority bracket and the opponent is faster")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_LOPMONX) { Speed(300); Moves(MOVE_QUICK_ATTACK); }
        OPPONENT(SPECIES_LOPMONX) { Speed(100); Moves(MOVE_SUCKER_PUNCH, MOVE_SCRATCH); }
    } WHEN {
