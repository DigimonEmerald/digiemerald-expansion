...
AI_DOUBLE_BATTLE_TEST("AI won't use a Weather changing move if partner already chose such move")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_LOPMON_X);
        OPPONENT(SPECIES_LOPMON_X) { Moves(weatherMoveLeft); }
        OPPONENT(SPECIES_LOPMON_X) { Moves(MOVE_SCRATCH, weatherMoveRight); }
    } WHEN {
...
AI_DOUBLE_BATTLE_TEST("AI will not use Helping Hand if partner does not have any damage moves")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_LOPMON_X);
        PLAYER(SPECIES_LOPMON_X);
        OPPONENT(SPECIES_LOPMON_X) { Moves(MOVE_HELPING_HAND, MOVE_SCRATCH); }
        OPPONENT(SPECIES_LOPMON_X) { Moves(move1, move2, move3, move4); }
    } WHEN {
        ...
        NOT MESSAGE("The opposing Lopmon_x used Helping Hand!");
    }
}
...
AI_DOUBLE_BATTLE_TEST("AI will not use a status move if partner already chose Helping Hand")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_LOPMON_X);
        PLAYER(SPECIES_LOPMON_X);
        OPPONENT(SPECIES_LOPMON_X) { Moves(MOVE_HELPING_HAND); }
        OPPONENT(SPECIES_LOPMON_X) { Moves(MOVE_SCRATCH, statusMove); }
    } WHEN {
        ...
        MESSAGE("The opposing Lopmon_x used Helping Hand!");
    }
}
...
AI_DOUBLE_BATTLE_TEST("AI will not choose Earthquake if it damages the partner without a positive effect")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_LOPMON_X);
        PLAYER(SPECIES_LOPMON_X);
        OPPONENT(SPECIES_PULSEMON) { Moves(MOVE_EARTHQUAKE, MOVE_SCRATCH); }
...
AI_DOUBLE_BATTLE_TEST("AI will choose Bulldoze if it triggers its ally's ability but will not KO the ally needlessly")
{
    ...
    PARAMETRIZE { species = SPECIES_MORPHOMON;   ability = ABILITY_CONTRARY; currentHP = 400; }
    ...
    PARAMETRIZE { species = SPECIES_MORPHOMON;   ability = ABILITY_CONTRARY; currentHP = 1; }

    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT);
        PLAYER(SPECIES_LOPMON_X);
        PLAYER(SPECIES_LOPMON_X);
        OPPONENT(SPECIES_PULSEMON) { Moves(MOVE_BULLDOZE, MOVE_HIGH_HORSEPOWER); }
...
AI_DOUBLE_BATTLE_TEST("AI will choose Beat Up on an ally with Justified if it will benefit the ally")
{
    ...
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT);
        PLAYER(SPECIES_LOPMON_X);
        PLAYER(SPECIES_PYONMON);
        OPPONENT(SPECIES_PANGORO)   { Ability(atkAbility); Moves(MOVE_BEAT_UP); }
        OPPONENT(SPECIES_FRIMON) { Moves(MOVE_CELEBRATE, MOVE_TACKLE); HP(currentHP); Ability(defAbility); }
    } WHEN {
...
AI_DOUBLE_BATTLE_TEST("AI will choose Earthquake if partner is not alive")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_LOPMON_X);
        PLAYER(SPECIES_LOPMON_X);
        OPPONENT(SPECIES_LOPMON_X) { Moves(MOVE_EARTHQUAKE, MOVE_SCRATCH); }
...
AI_DOUBLE_BATTLE_TEST("AI will choose Earthquake if it kills one opposing mon and does not kill the partner needlessly")
{
    ...
    GIVEN {
        ...
        OPPONENT(SPECIES_LOPMON_X) { Moves(MOVE_EARTHQUAKE, MOVE_SCRATCH); }
        OPPONENT(SPECIES_ARGOMON_IT) { Moves(MOVE_CELEBRATE); HP(currentHP); }
    } WHEN {
        if (currentHP == 1)
...
AI_DOUBLE_BATTLE_TEST("AI will choose Earthquake if it kills one opposing mon and a partner it believes is about to die")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_LOPMON_X) { Moves(MOVE_CELEBRATE, MOVE_SCRATCH); }
        PLAYER(SPECIES_LOPMON_X) { HP(1); Moves(MOVE_CELEBRATE, MOVE_SCRATCH); }
        OPPONENT(SPECIES_LOPMON_X) { Moves(MOVE_EARTHQUAKE, MOVE_SCRATCH); }
        OPPONENT(SPECIES_ARGOMON_IT) { Moves(MOVE_CELEBRATE); HP(1); }
    } WHEN {
...
AI_DOUBLE_BATTLE_TEST("AI will trigger its ally's Weakness Policy")
{
    ...
    PARAMETRIZE { species = SPECIES_GUMDRAMON; }
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_LOPMON_X);
        PLAYER(SPECIES_LOPMON_X);
        OPPONENT(SPECIES_LOPMON_X) { Moves(MOVE_EARTHQUAKE, MOVE_STOMPING_TANTRUM); }
...
AI_DOUBLE_BATTLE_TEST("AI will only explode and kill everything on the field with Risky or Will Suicide")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_LOPMON_X) { HP(1); }
        PLAYER(SPECIES_LOPMON_X) { HP(1); }
        OPPONENT(SPECIES_YAAMON) { Moves(MOVE_EXPLOSION, MOVE_ELECTRO_BALL); }
        OPPONENT(SPECIES_YAAMON) { Moves(MOVE_CELEBRATE); HP(1); }
    } WHEN {
...
AI_DOUBLE_BATTLE_TEST("AI sees corresponding absorbing abilities on partners")
{
    ...
    PARAMETRIZE { species = SPECIES_CHICCHIMON;    ability = ABILITY_CLOUD_NINE;         move = MOVE_DISCHARGE; }
    PARAMETRIZE { species = SPECIES_PETITMON;    ability = ABILITY_LIGHTNING_ROD;      move = MOVE_DISCHARGE; }
    PARAMETRIZE { species = SPECIES_GUILMON;    ability = ABILITY_VOLT_ABSORB;        move = MOVE_DISCHARGE; }
    PARAMETRIZE { species = SPECIES_EMOLGA;     ability = ABILITY_MOTOR_DRIVE;        move = MOVE_DISCHARGE; }
    PARAMETRIZE { species = SPECIES_BLKGABUMON;    ability = ABILITY_LIGHTNING_ROD;      move = MOVE_DISCHARGE; }
    PARAMETRIZE { species = SPECIES_FRIMON;  ability = ABILITY_FLASH_FIRE;         move = MOVE_LAVA_PLUME; }
    PARAMETRIZE { species = SPECIES_DACHSBUN;   ability = ABILITY_WELL_BAKED_BODY;    move = MOVE_LAVA_PLUME; }
    PARAMETRIZE { species = SPECIES_KUDAMON_06;   ability = ABILITY_WATER_ABSORB;       move = MOVE_SURF; }
    PARAMETRIZE { species = SPECIES_SHELLOS;    ability = ABILITY_STORM_DRAIN;        move = MOVE_SURF; }
    PARAMETRIZE { species = SPECIES_LOPMON;      ability = ABILITY_LEVITATE;           move = MOVE_EARTHQUAKE; }
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_TOYAGUMON);
        OPPONENT(SPECIES_BABOONMON) { Moves(move, MOVE_SCRATCH); }
...
AI_DOUBLE_BATTLE_TEST("AI knows if redirection abilities provide immunity to allies")
{
    ...
    PARAMETRIZE { species = SPECIES_BLKGABUMON;    ability = ABILITY_LIGHTNING_ROD;    move = MOVE_DISCHARGE;  config = GEN_4; }
    PARAMETRIZE { species = SPECIES_BLKGABUMON;    ability = ABILITY_LIGHTNING_ROD;    move = MOVE_DISCHARGE;  config = GEN_5; }
