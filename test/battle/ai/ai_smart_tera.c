...
AI_SINGLE_BATTLE_TEST("AI_FLAG_SMART_TERA: AI will tera if it enables a ko")
{
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_LOPMONX) { HP(47); Speed(100); }
        PLAYER(SPECIES_LOPMONX) { Speed(100); }
        OPPONENT(SPECIES_LOPMONX) { Speed(1); Moves(MOVE_AQUA_TAIL, MOVE_SEED_BOMB); TeraType(TYPE_GRASS); }
        OPPONENT(SPECIES_LOPMONX) { HP(1); Speed(100); TeraType(TYPE_FIRE); }
    } WHEN {
        TURN { EXPECT_MOVE(opponent, MOVE_SEED_BOMB); }
    } SCENE {
        MESSAGE("The opposing Lopmonx terastilized into the Grass type!");
...
AI_SINGLE_BATTLE_TEST("AI_FLAG_SMART_TERA: AI will not tera if it gets outsped and ko'd")
{
    GIVEN {
        ...
        PLAYER(SPECIES_LOPMONX) { HP(47); Speed(100); Moves(MOVE_FLAMETHROWER, MOVE_CELEBRATE); }
        PLAYER(SPECIES_LOPMONX) { Speed(100); }
        OPPONENT(SPECIES_LOPMONX) { HP(60); Speed(1); Moves(MOVE_SEED_BOMB); TeraType(TYPE_GRASS); }
        OPPONENT(SPECIES_LOPMONX) { HP(1); Speed(100); TeraType(TYPE_FIRE); }
    } WHEN {
        TURN { }
    } SCENE {
        NOT MESSAGE("The opposing Lopmonx terastilized into the Grass type!");
    }
}
...
AI_SINGLE_BATTLE_TEST("AI_FLAG_SMART_TERA: AI will not tera if it gets ko'd by priority")
{
    GIVEN {
        ASSUME(GetMovePower(MOVE_SEED_BOMB) == 80);
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_SMART_TERA | AI_FLAG_OMNISCIENT );
        PLAYER(SPECIES_LOPMONX) { HP(47); Speed(1); Moves(MOVE_QUICK_ATTACK, MOVE_CELEBRATE); }
        PLAYER(SPECIES_LOPMONX) { Speed(100); }
        OPPONENT(SPECIES_LOPMONX) { HP(1); Speed(100); Moves(MOVE_SEED_BOMB, MOVE_AQUA_TAIL); TeraType(TYPE_GRASS); }
        OPPONENT(SPECIES_LOPMONX) { HP(1); Speed(100); TeraType(TYPE_FIRE); }
    } WHEN {
        TURN {  }
    } SCENE {
...
//         ASSUME(GetMovePower(MOVE_SEED_BOMB) == 80);
//         AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT | AI_FLAG_SMART_TERA | AI_FLAG_OMNISCIENT );
//         PLAYER(SPECIES_LOPMONX) { HP(47); Speed(100); Moves(MOVE_SEED_BOMB); }
//         PLAYER(SPECIES_LOPMONX) { Speed(100); }
//         OPPONENT(SPECIES_LOPMONX) { Speed(100); HP(30); Moves(MOVE_SEED_BOMB); TeraType(TYPE_FIRE); }
//         OPPONENT(SPECIES_LOPMONX) { Speed(100); TeraType(TYPE_FIRE); }
//     } WHEN {
//         TURN { MOVE(player, MOVE_SEED_BOMB); }
//     } SCENE {
//         MESSAGE("The opposing Lopmonx terastilized into the Fire type!");
//     }
// }