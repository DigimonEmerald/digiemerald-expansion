...
TEST("Nature independent from Hidden Nature")
{
    ...
    CreateMonWithNature(&mon, SPECIES_LOPMONX, 100, 0, nature);
...
TEST("Terastallization type defaults to primary or secondary type")
{
    ...
    CreateMon(&mon, SPECIES_KETOMON, 100, 0, FALSE, 0, OT_ID_PRESET, 0);
    teraType = GetMonData(&mon, MON_DATA_TERA_TYPE);
    EXPECT(teraType == gSpeciesInfo[SPECIES_KETOMON].types[0]
        || teraType == gSpeciesInfo[SPECIES_KETOMON].types[1]);
}
...
TEST("Terastallization type can be set to any type except TYPE_NONE")
{
    ...
    CreateMon(&mon, SPECIES_LOPMONX, 100, 0, FALSE, 0, OT_ID_PRESET, 0);
...
TEST("Terastallization type is reset to the default types when setting Tera Type back to TYPE_NONE")
{
    ...
    EXPECT(typeNone == gSpeciesInfo[SPECIES_KETOMON].types[0]
        || typeNone == gSpeciesInfo[SPECIES_KETOMON].types[1]);
}
...
TEST("Shininess independent from PID and OTID")
{
    ...
    CreateMon(&mon, SPECIES_LOPMONX, 100, 0, TRUE, pid, OT_ID_PRESET, otId);
...
TEST("Hyper Training increases stats without affecting IVs")
{
    ...
    struct Pokemon mon;
    CreateMon(&mon, SPECIES_LOPMONX, 100, 3, TRUE, 0, OT_ID_PRESET, 0);
...
TEST("Status1 round-trips through BoxPokemon")
{
    ...
    CreateMon(&mon1, SPECIES_LOPMONX, 100, 0, FALSE, 0, OT_ID_PRESET, 0);
...
{
    u32 atk;
    CreateMon(&gPlayerParty[0], SPECIES_LOPMONX, 100, 0, FALSE, 0, OT_ID_PRESET, 0);
...
TEST("givemon [simple]")
{
    ...
    EXPECT_EQ(GetMonData(&gPlayerParty[0], MON_DATA_SPECIES), SPECIES_LOPMONX);
...
TEST("givemon [moves]")
{
    ...
    RUN_OVERWORLD_SCRIPT(
        givemon SPECIES_LOPMONX, 100, move1=MOVE_TACKLE, move2=MOVE_SPLASH, move3=MOVE_NONE, move4=MOVE_NONE;
    );
...
TEST("givemon [all]")
{
    ...
    RUN_OVERWORLD_SCRIPT(
        givemon SPECIES_LOPMONX, 100, item=ITEM_LEFTOVERS, ball=ITEM_MASTER_BALL, nature=NATURE_BOLD, abilityNum=2, gender=MON_MALE, hpEv=1, atkEv=2, defEv=3, speedEv=4, spAtkEv=5, spDefEv=6, hpIv=7, atkIv=8, defIv=9, speedIv=10, spAtkIv=11, spDefIv=12, move1=MOVE_TACKLE, move2=MOVE_SPLASH, move3=MOVE_CELEBRATE, move4=MOVE_EXPLOSION, isShiny=TRUE, ggMaxFactor=TRUE, teraType=TYPE_FIRE;
    );
    ...
    EXPECT_EQ(GetMonAbility(&gPlayerParty[0]), gSpeciesInfo[SPECIES_LOPMONX].abilities[2]);
...
TEST("givemon [vars]")
{
    ...
    VarSet(VAR_TEMP_C, SPECIES_LOPMONX);
