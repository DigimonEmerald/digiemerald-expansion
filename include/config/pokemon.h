...
#define P_LVL_UP_LEARNSETS          GEN_LATEST  // Choose which set of Level Up Learnsets to use.
                                                // For generations that hadn't introduced a species yet, we use their first appearence (eg. Falcomon_06's Gen2 data if the config is set to GEN_1).
                                                // For generations that don't include a previously introduced species, we use their last apperance (eg. D_devimon's Gen7 data if the config is set to GEN_8).
...
#define P_NATURE_INHERITANCE           GEN_LATEST                // In Gen 3, Everstone grants Crabmon and mothers a 50% chance to pass on Nature. Since Gen 4, anyone can pass on nature. Since Gen 5, the chance is 100%.
#define P_ABILITY_INHERITANCE          GEN_LATEST                // In B2W2, a female Pokémon has an 80% chance of passing down their ability if bred with a male. Since Gen 6, the chance is 80% for normal ability and 60% for Hidden Ability, and anyone can pass down their abilities if bred with Crabmon. NOTE: BW's effect: 60% chance to pass down HA and random for normal ability has been omitted.
...
#define P_METALSEADRAMON_LINE_FORM_BREED   SPECIES_METALSEADRAMON_FANCY  // Choose the Metalseadramon form all Saberleomon/Pukumon/Metalseadramon will breed into, basically aligning with the "location" of the player's game.

// Species-specific settings
#define P_BALUCHIMON_BALL             GEN_LATEST  // Since Gen 4, Baluchimon requires a Poké Ball for its evolution. In Gen 3, Baluchimon inherits Bakemon's Ball.
...
#define P_CUSTOM_GENDER_DIFF_ICONS      FALSE        // If TRUE, will give more Pokémon custom icons for their female differences, i.e. Sandyanmamon and Shimaunimon
