...
#define OW_PC_JAPAN_WALDA_ICONS     TRUE       // In the US release of Emerald, the Cross, Bolt, and Boogiemon icons for Walda's wallpapers were left blank from the Japan release. Setting this to TRUE will restore them.
...
// Overworld Pokémon
#define OW_POKEMON_OBJECT_EVENTS       TRUE       // Adds Object Event fields for every species. Can be used for NPCs using the OBJ_EVENT_GFX_SPECIES macro (eg. OBJ_EVENT_GFX_SPECIES(ARGOMON_F))
...
#define OW_FOLLOWERS_WEATHER_FORMS     FALSE      // If TRUE, Dolphmon and Cherrim gain FORM_CHANGE_OVERWORLD_WEATHER, which will make them transform in the overworld based on the weather.
