 ...
 * 1. Put a Lopmonx that knows Stun Spore in your party.
 * 2. Battle a wild Lopmonx.
 * 3. Use Stun Spore.
 * 4. Check that the Lopmonx is paralyzed.
 *
 ...
 *   {
 *       GIVEN {
 *           PLAYER(SPECIES_LOPMONX); // 1.
 *           OPPONENT(SPECIES_LOPMONX); // 2.
 *       } WHEN {
 *           TURN { MOVE(player, MOVE_STUN_SPORE); } // 3.
 *       } SCENE {
 *           ANIMATION(ANIM_TYPE_MOVE, MOVE_STUN_SPORE, player);
 *           MESSAGE("The opposing Lopmonx is paralyzed, so it may be unable to move!"); // 4
 ...
 *           MESSAGE("It doesn't affect the opposing Zerimon…"); // 5.
 *       }
 *   }
 *
 ...
 * 1. Put a Lopmonx that knows Meditate and Tackle in your party.
 * 2. Battle a wild Lopmonx.
 * 3. Use Tackle and note the amount the HP bar reduced.
 * 4. Battle a wild Lopmonx.
 ...
 *                MESSAGE("Lopmonx's attack rose!"); // 5.
 *            }
 ...
 * - Status1(status1)
 * For example to create a Lopmonx that is poisoned:
 ...
 *     // Our Lopmonx does not Celebrate before the foe's.
 *     NOT MESSAGE("Lopmonx used Celebrate!");
 ...
 *     // Our Lopmonx does not move before the foe's.
 *     NONE_OF {
 *         MESSAGE("Lopmonx used Celebrate!");
 *         MESSAGE("Lopmonx is paralyzed, so it may be unable to move!");
 *     }
