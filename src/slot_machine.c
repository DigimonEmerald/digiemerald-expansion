...
#define BIAS_REGULAR (BIAS_REPLAY | BIAS_CHERRY | BIAS_LOATAD | BIAS_BIOSTEGOMON | BIAS_POWER)
...
struct SlotMachine
{
    ...
    /*0x3F*/ u8 reelTimePetitmonSpriteId;
    ...
    /*0x52*/ u8 reelTimePetitmonAuraSpriteIds[2];
...
static void ReelTime_WaitStartPetitmon(struct Task *);
static void ReelTime_PetitmonSpeedUp1(struct Task *);
static void ReelTime_PetitmonSpeedUp2(struct Task *);
...
static void ReelTime_PetitmonReact(struct Task *);
...
static void CreateReelTimePetitmonSprite(void);
static void DestroyReelTimePetitmonSprite(void);
...
static void CreateReelTimePetitmonAuraSprites(void);
static void SetReelTimePetitmonAuraFlashDelay(s16);
static void DestroyReelTimePetitmonAuraSprites(void);
...
static void SpriteCB_ReelTimePetitmon(struct Sprite *);
static void SpriteCB_ReelTimeNumbers(struct Sprite *);
static void SpriteCB_ReelTimeBolt(struct Sprite *);
static void SpriteCB_ReelTimePetitmonAura(struct Sprite *);
...
static EWRAM_DATA struct SpriteFrameImage *sImageTable_ReelTimePetitmon = NULL;
...
static const struct SpriteTemplate sSpriteTemplate_ReelTimePetitmonAura;
...
static const struct SpriteTemplate sSpriteTemplate_ReelTimePetitmon;
...
static void (*const sReelTimeTasks[])(struct Task *task) =
{
    ...
    [RT_TASK_WAIT_START_PIKA]      = ReelTime_WaitStartPetitmon,
    [RT_TASK_PIKA_SPEEDUP1]        = ReelTime_PetitmonSpeedUp1,
    [RT_TASK_PIKA_SPEEDUP2]        = ReelTime_PetitmonSpeedUp2,
    [RT_TASK_WAIT_REEL]            = ReelTime_WaitReel,
    [RT_TASK_CHECK_EXPLODE]        = ReelTime_CheckExplode,
    [RT_TASK_LAND]                 = ReelTime_LandOnOutcome,
    [RT_TASK_PIKA_REACT]           = ReelTime_PetitmonReact,
...
static const u8 sReelTimePetitmonAnimIds[] = {1, 1, 2, 2};
static const s16 sReelTimeBoltDelays[] = {64, 48, 24, 8};
static const s16 sPetitmonAuraFlashDelays[] = {10, 8, 6, 4};
...
static bool8 SlotTask_FreeDataStructures(struct Task *task)
{
    if (!gPaletteFade.active)
    {
        ...
        TRY_FREE_AND_SET_NULL(sImageTable_ReelTimePetitmon);
...
static void ReelTime_WaitStartPetitmon(struct Task *task)
{
    AdvanceReeltimeReel(task->tRtReelSpeed >> 8);
    if (++task->tTimer1 >= 60)
    {
        ...
        CreateReelTimePetitmonAuraSprites();
    }
}

static void ReelTime_PetitmonSpeedUp1(struct Task *task)
{
    int i;
    u8 petitmonAnimIds[ARRAY_COUNT(sReelTimePetitmonAnimIds)];
    s16 reelTimeBoltDelays[ARRAY_COUNT(sReelTimeBoltDelays)];
    s16 petitmonAuraFlashDelays[ARRAY_COUNT(sPetitmonAuraFlashDelays)];

    memcpy(petitmonAnimIds, sReelTimePetitmonAnimIds, sizeof(sReelTimePetitmonAnimIds));
    memcpy(reelTimeBoltDelays, sReelTimeBoltDelays, sizeof(sReelTimeBoltDelays));
    memcpy(petitmonAuraFlashDelays, sPetitmonAuraFlashDelays, sizeof(sPetitmonAuraFlashDelays));
    ...
    SetReelTimePetitmonAuraFlashDelay(petitmonAuraFlashDelays[i]);
    StartSpriteAnimIfDifferent(&gSprites[sSlotMachine->reelTimePetitmonSpriteId], petitmonAnimIds[i]);
...
static void ReelTime_PetitmonSpeedUp2(struct Task *task)
{
    AdvanceReeltimeReel(task->tRtReelSpeed >> 8);
    if (++task->tTimer1 >= 80)
    {
        ...
        task->tTimer1 = 0;
        SetReelTimePetitmonAuraFlashDelay(2);
        StartSpriteAnimIfDifferent(&gSprites[sSlotMachine->reelTimePetitmonSpriteId], 3);
    }
}
...
// Animate Petitmon reaction. Clear any power bolts the player may have won if
// they got a positive ReelTime draw.
static void ReelTime_PetitmonReact(struct Task *task)
{
    if (++task->tTimer2 >= 60)
    {
        StopMapMusic();
        DestroyReelTimeBoltSprites();
        DestroyReelTimePetitmonAuraSprites();
        task->tState++; // RT_TASK_WAIT_CLEAR_POWER
        if(sSlotMachine->reelTimeDraw == 0)
        {
            task->tTimer2 = 0xa0;
            StartSpriteAnimIfDifferent(&gSprites[sSlotMachine->reelTimePetitmonSpriteId], 5);
            PlayFanfare(MUS_TOO_BAD);
        }
        else
        {
            task->tTimer2 = 0xc0;
            StartSpriteAnimIfDifferent(&gSprites[sSlotMachine->reelTimePetitmonSpriteId], 4);
            gSprites[sSlotMachine->reelTimePetitmonSpriteId].animCmdIndex = 0;
...
static void CreateReelTimePetitmonSprite(void)
{
    ...
    u8 spriteId;
    if (sImageTable_ReelTimePetitmon == NULL)
        sImageTable_ReelTimePetitmon = AllocZeroed(sizeof(struct SpriteFrameImage) * 5);

    sImageTable_ReelTimePetitmon[0].data = sReelTimeGfxPtr + (0 * 0x800);
    sImageTable_ReelTimePetitmon[0].size = 0x800;
    sImageTable_ReelTimePetitmon[1].data = sReelTimeGfxPtr + (1 * 0x800);
    sImageTable_ReelTimePetitmon[1].size = 0x800;
    sImageTable_ReelTimePetitmon[2].data = sReelTimeGfxPtr + (2 * 0x800);
    sImageTable_ReelTimePetitmon[2].size = 0x800;
    sImageTable_ReelTimePetitmon[3].data = sReelTimeGfxPtr + (3 * 0x800);
    sImageTable_ReelTimePetitmon[3].size = 0x800;
    sImageTable_ReelTimePetitmon[4].data = sReelTimeGfxPtr + (4 * 0x800);
    sImageTable_ReelTimePetitmon[4].size = 0x800;

    spriteTemplate = sSpriteTemplate_ReelTimePetitmon;
    spriteTemplate.images = sImageTable_ReelTimePetitmon;
    spriteId = CreateSprite(&spriteTemplate, 280, 80, 1);
    gSprites[spriteId].oam.priority = 1;
    gSprites[spriteId].coordOffsetEnabled = TRUE;
    sSlotMachine->reelTimePetitmonSpriteId = spriteId;
}

static void DestroyReelTimePetitmonSprite(void)
{
    DestroySprite(&gSprites[sSlotMachine->reelTimePetitmonSpriteId]);
    TRY_FREE_AND_SET_NULL(sImageTable_ReelTimePetitmon);
}

static void SpriteCB_ReelTimePetitmon(struct Sprite *sprite)
{
...
static void CreateReelTimePetitmonAuraSprites(void)
{
    ...
    u8 spriteId = CreateSprite(&sSpriteTemplate_ReelTimePetitmonAura, 72, 80, 3);
    gSprites[spriteId].oam.priority = 1;
    ...
    sSlotMachine->reelTimePetitmonAuraSpriteIds[0] = spriteId;

    // Right half
    spriteId = CreateSprite(&sSpriteTemplate_ReelTimePetitmonAura, 104, 80, 3);
    gSprites[spriteId].oam.priority = 1;
    gSprites[spriteId].hFlip = TRUE;
    sSlotMachine->reelTimePetitmonAuraSpriteIds[1] = spriteId;
}

static void SpriteCB_ReelTimePetitmonAura(struct Sprite *sprite)
{
    u8 colors[] = {16, 0};
...
static void SetReelTimePetitmonAuraFlashDelay(s16 delay)
{
    gSprites[sSlotMachine->reelTimePetitmonAuraSpriteIds[0]].sDelay = delay;
}
...
static void DestroyReelTimePetitmonAuraSprites(void)
{
    ...
    for (i = 0; i < ARRAY_COUNT(sSlotMachine->reelTimePetitmonAuraSpriteIds); i++)
        DestroySprite(&gSprites[sSlotMachine->reelTimePetitmonAuraSpriteIds[i]]);
}
...
// The "confusion" ducks that circle Petitmon if the Reel Time machine explodes
static void CreateReelTimeDuckSprites(void)
{
    u8 i;
...
static const u8 sBiasProbabilities_Regular[][NUM_SLOT_MACHINE_IDS] = {
    ...
    {
        // Probabilities for BIAS_BIOSTEGOMON
    ...
    {
        // Probabilities for BIAS_ZUBAMON
        [SLOT_MACHINE_UNLUCKIEST] = 25,
        [SLOT_MACHINE_UNLUCKIER]  = 25,
        [SLOT_MACHINE_UNLUCKY]    = 25,
        [SLOT_MACHINE_LUCKY]      = 30,
...
static const u8 sBiasSymbols[] = {
  ...
  SYMBOL_BIOSTEGOMON, // BIAS_BIOSTEGOMON
...
static const u16 sBiasesRegular[] = {
    BIAS_POWER, BIAS_BIOSTEGOMON, BIAS_ZUBAMON, BIAS_CHERRY, BIAS_REPLAY
};

static const u8 sSymbolToMatch[] = {
    [SYMBOL_7_RED]   = MATCH_RED_7,
    [SYMBOL_7_BLUE]  = MATCH_BLUE_7,
    [SYMBOL_BIOSTEGOMON] = MATCH_BIOSTEGOMON,
...
static const u16 sSlotMatchFlags[] = {
    ...
    [MATCH_BIOSTEGOMON]       = 1 << MATCH_BIOSTEGOMON,
...
static const struct SpriteFrameImage sImageTable_ReelTimePetitmonAura[] = { {gSlotMachineReelTimePikaAura, 0x400} };
...
static const union AnimCmd sAnim_ReelTimePetitmon_Still[] =
{
    ANIMCMD_FRAME(0, 16),
    ANIMCMD_END
};

static const union AnimCmd sAnim_ReelTimePetitmon_ChargingSlow[] =
{
    ANIMCMD_FRAME(1, 16),
    ANIMCMD_FRAME(0, 16),
    ANIMCMD_JUMP(0)
...
static const union AnimCmd sAnim_ReelTimePetitmon_ChargingMedium[] =
{
    ANIMCMD_FRAME(1, 8),
    ANIMCMD_FRAME(0, 8),
    ANIMCMD_JUMP(0)
...
static const union AnimCmd sAnim_ReelTimePetitmon_ChargingFast[] =
{
    ANIMCMD_FRAME(1, 4),
    ANIMCMD_FRAME(0, 4),
    ANIMCMD_JUMP(0)
...
static const union AnimCmd sAnim_ReelTimePetitmon_Cheering[] =
{
    ANIMCMD_FRAME(2, 32),
    ANIMCMD_FRAME(3, 32),
    ANIMCMD_JUMP(0)
...
static const union AnimCmd sAnim_ReelTimePetitmon_FellOver[] =
{
    ANIMCMD_FRAME(4, 1),
    ANIMCMD_END
};
...
static const union AnimCmd *const sAnims_ReelTimePetitmon[] =
{
    sAnim_ReelTimePetitmon_Still,
    sAnim_ReelTimePetitmon_ChargingSlow,
    sAnim_ReelTimePetitmon_ChargingMedium,
    sAnim_ReelTimePetitmon_ChargingFast,
    sAnim_ReelTimePetitmon_Cheering,
    sAnim_ReelTimePetitmon_FellOver
};
...
{
    .tileTag = TAG_NONE,
    .paletteTag = PALTAG_REEL_TIME_PETITMON,
    .oam = &sOam_64x64,
    .anims = sAnims_ReelTimePetitmon,
    .images = NULL,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCB_ReelTimePetitmon
};
...
static const struct SpriteTemplate sSpriteTemplate_ReelTimePetitmonAura =
{
    ...
    .oam = &sOam_32x64,
    .anims = sAnims_SingleFrame,
    .images = sImageTable_ReelTimePetitmonAura,
    .affineAnims = gDummySpriteAffineAnimTable,
    .callback = SpriteCB_ReelTimePetitmonAura
};
...
static const struct SpriteSheet sSlotMachineSpriteSheets[22] =
{
    ...
    { .data = gSlotMachineReelSymbol3Tiles, .size = 0x200, .tag = GFXTAG_BIOSTEGOMON },
...
static const struct SpritePalette sSlotMachineSpritePalettes[] =
{
    ...
    { .data = gSlotMachineReelTimePetitmon_Pal,   .tag = PALTAG_REEL_TIME_PETITMON},
...
static const u32 sReelTimeGfx[] = INCBIN_U32("graphics/slot_machine/reel_time_gfx.4bpp.smol"); // reel_time_machine and reel_time_petitmon
