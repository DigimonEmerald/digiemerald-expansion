    ...
    if (GetMoveEffect(ctx->currentMove) == EFFECT_MULTI_HIT)
    {
        ...
        if (ability == ABILITY_SKILL_LINK)
        {
            gMultiHitCounter = 5;
              ...
              && gBattleMons[ctx->battlerAtk].species == SPECIES_ROSEMON_BURSTMODE)
        {
            gMultiHitCounter = 3;
        }
        else
    ...
    if (battleScriptBlocksMove == NULL)
    {
        ...
        if (atkPriority <= 0)
        {
            // Not a priority move
              ...
              && !(IsBattleMoveStatus(move) && (abilityDef == ABILITY_MAGIC_BOUNCE || gProtectStructs[battlerDef].bounceMove || abilityDef == ABILITY_SIN_OF_WRATH)))
        {
...
u32 AbilityBattleEffects(enum AbilityEffect caseID, u32 battler, enum Ability ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_ON_SWITCHIN:
        ...
        case ABILITY_COMMANDER:
            ...
            if (!gSpecialStatuses[battler].switchInAbilityDone
             ...
             && gBattleMons[partner].species == SPECIES_ATAMADEKACHIMON
             && GET_BASE_SPECIES_ID(GetMonData(GetBattlerMon(battler), MON_DATA_SPECIES)) == SPECIES_MINIDEKACHIMON)
            {
                SaveBattlerAttacker(gBattlerAttacker);
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
        ...
        case ABILITY_DEATH_EVOLUTION:
            if (!gSpecialStatuses[battler].switchInAbilityDone && !IsOpposingSideEmpty(battler))
            {
                ...
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                SET_STATCHANGER(STAT_ATK, 1, TRUE);
                BattleScriptPushCursorAndCallback(BattleScript_DeathEvolutionActivates);
                effect++;
            }
            break;
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        case ABILITY_TANGLING_HAIR:
            if (IsBattlerAlive(gBattlerAttacker)
             && (CompareStat(gBattlerAttacker, STAT_SPEED, MIN_STAT_STAGE, CMP_GREATER_THAN, gLastUsedAbility) || GetBattlerAbility(gBattlerAttacker) == ABILITY_MIRROR_ARMOR || GetBattlerAbility(gBattlerAttacker) == ABILITY_SIN_OF_WRATH)
    ...
    case ABILITYEFFECT_MOVE_END_ATTACKER: // Same as above, but for attacker
        ...
        case ABILITY_HIGH_HAZE:
            ...
            break;
        case ABILITY_SEALED_FISTS:
            if (!gBattleMons[gBattlerAttacker].volatiles.healBlock
                     && gBattleStruct->moveDamage[gBattlerTarget] > 0
                     && IsBattlerAlive(gBattlerAttacker))
                {
                    gBattleStruct->moveDamage[gBattlerAttacker] = max(1, (gBattleStruct->moveDamage[gBattlerTarget] * 25 / 100));
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    case ABILITY_SOUND_BOOST:
        if (IsSoundMove(move))
           modifier = uq4_12_multiply(modifier, UQ_4_12(1.2));
        break;
    case ABILITY_THUNDER_BLADE:
        if (IsSlicingMove(move))
           modifier = uq4_12_multiply(modifier, UQ_4_12(1.2));
        break;
    case ABILITY_BIG_HORN:
        if (IsHornMove(move))
           modifier = uq4_12_multiply(modifier, UQ_4_12(1.2));
        break;
    case ABILITY_STRONG_LEGS:
        if (IsKickMove(move))
           modifier = uq4_12_multiply(modifier, UQ_4_12(1.2));
        break;
    case ABILITY_GREAT_PURGE:
...
static inline u32 CalcAttackStat(struct DamageContext *ctx)
{
    ...
    case ABILITY_MINUS:
        ...
        break;
    case ABILITY_FLOWER_GIFT:
        if (gBattleMons[battlerAtk].species == SPECIES_VEEDRAMON && IsBattlerWeatherAffected(battlerAtk, B_WEATHER_SUN) && IsBattleMovePhysical(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_HUSTLE:
        if (IsBattleMovePhysical(move))
    ...
    if (IsBattlerAlive(BATTLE_PARTNER(battlerAtk)))
    {
        ...
        {
        case ABILITY_FLOWER_GIFT:
            if (gBattleMons[BATTLE_PARTNER(battlerAtk)].species == SPECIES_VEEDRAMON && IsBattlerWeatherAffected(BATTLE_PARTNER(battlerAtk), B_WEATHER_SUN) && IsBattleMovePhysical(move))
                modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
            break;
        default:
            break;
    ...
    case HOLD_EFFECT_THICK_CLUB:
        if ((atkBaseSpeciesId == SPECIES_AGUMON_06 || atkBaseSpeciesId == SPECIES_AGUMON_EXPERT) && IsBattleMovePhysical(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
    case HOLD_EFFECT_DEEP_SEA_TOOTH:
        if (gBattleMons[battlerAtk].species == SPECIES_FIRAMON && IsBattleMoveSpecial(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
    case HOLD_EFFECT_LIGHT_BALL:
        if (atkBaseSpeciesId == SPECIES_PETITMON && (B_LIGHT_BALL_ATTACK_BOOST >= GEN_4 || IsBattleMoveSpecial(move)))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
...
static inline u32 CalcDefenseStat(struct DamageContext *ctx)
{
    ...
    case ABILITY_GRASS_PELT:
        ...
        break;
    case ABILITY_FLOWER_GIFT:
        if (gBattleMons[battlerDef].species == SPECIES_VEEDRAMON && IsBattlerWeatherAffected(battlerDef, B_WEATHER_SUN) && !usesDefStat)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    default:
        break;
    ...
    if (IsBattlerAlive(BATTLE_PARTNER(battlerDef)))
    {
        ...
        {
        case ABILITY_FLOWER_GIFT:
            if (gBattleMons[BATTLE_PARTNER(battlerDef)].species == SPECIES_VEEDRAMON && IsBattlerWeatherAffected(BATTLE_PARTNER(battlerDef), B_WEATHER_SUN) && !usesDefStat)
                modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
            break;
        default:
            break;
    ...
    case HOLD_EFFECT_METAL_POWDER:
        if (gBattleMons[battlerDef].species == SPECIES_CRABMON && usesDefStat && !(gBattleMons[battlerDef].volatiles.transformed))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
...
static inline s32 DoMoveDamageCalcVars(struct DamageContext *ctx)
{
    ...
    if (gBattleMons[ctx->battlerAtk].attribute == TYPE_VACCINE && gBattleMons[ctx->battlerDef].attribute == TYPE_VIRUS)
        dmg = dmg * 15 / 10;
    if (gBattleMons[ctx->battlerAtk].attribute == TYPE_VIRUS && gBattleMons[ctx->battlerDef].attribute == TYPE_DATA)
        dmg = dmg * 15 / 10;
    if (gBattleMons[ctx->battlerAtk].attribute == TYPE_DATA && gBattleMons[ctx->battlerDef].attribute == TYPE_VACCINE)
        dmg = dmg * 15 / 10;
...
bool32 CanMoveSkipAccuracyCalc(u32 battlerAtk, u32 battlerDef, enum Ability abilityAtk, enum Ability abilityDef, u32 move, enum FunctionCallOption option)
{
    ...
    // If the attacker has the ability No Guard/Omega InForce and they aren't targeting a Pokemon involved in a Sky Drop with the move Sky Drop, move hits.
    ...
    else if (abilityAtk == ABILITY_OMEGA_INFORCE
          && gBattleMons[battlerDef].volatiles.semiInvulnerable != STATE_COMMANDER
          && (moveEffect != EFFECT_SKY_DROP || gBattleStruct->skyDropTargets[battlerDef] == SKY_DROP_NO_TARGET))
    {
        effect = TRUE;
