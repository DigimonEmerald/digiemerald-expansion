...
void LoadWallyToyagumon(void)
{
    u16 monData;
    CreateMon(&gPlayerParty[0], SPECIES_TOYAGUMON, 7, USE_RANDOM_IVS, FALSE, 0, OT_ID_PLAYER_ID, 0);
    monData = TRUE;
...
static const u8 *const sScrollableMultichoiceOptions[][MAX_SCROLL_MULTI_LENGTH] =
{
    ...
    [SCROLL_MULTI_BF_EXCHANGE_CORNER_DECOR_VENDOR_1] =
    {
        ...
        COMPOUND_STRING("SANGOMON DOLL{CLEAR_TO 0x5E}32BP"),
        COMPOUND_STRING("HAGURUMON DOLL{CLEAR_TO 0x5E}48BP"),
        COMPOUND_STRING("CALUMON DOLL{CLEAR_TO 0x5E}48BP"),
        COMPOUND_STRING("PUYOMON DOLL{CLEAR_TO 0x5E}48BP"),
        COMPOUND_STRING("CRABMON DOLL{CLEAR_TO 0x5E}48BP"),
        COMPOUND_STRING("FLORAMON DOLL{CLEAR_TO 0x5E}80BP"),
        COMPOUND_STRING("FALCOMON_06 DOLL{CLEAR_TO 0x5E}80BP"),
        COMPOUND_STRING("GAMMAMON DOLL{CLEAR_TO 0x5E}80BP"),
        gText_Exit
    },
    [SCROLL_MULTI_BF_EXCHANGE_CORNER_DECOR_VENDOR_2] =
    {
        COMPOUND_STRING("CORONAMON DOLL{CLEAR_TO 0x58}128BP"),
        COMPOUND_STRING("DRACOMON DOLL{CLEAR_TO 0x58}128BP"),
        COMPOUND_STRING("BOMMON DOLL{CLEAR_TO 0x58}256BP"),
        COMPOUND_STRING("CHIBOMON DOLL{CLEAR_TO 0x58}256BP"),
        COMPOUND_STRING("CURIMON DOLL{CLEAR_TO 0x58}256BP"),
        gText_Exit
    },
...
static const u16 sGesomonRockPalettes[GESOMON_ROCK_LEVELS][16] = {
...
static const u8 sGesomonRockCoords[GESOMON_ROCK_LEVELS][2] = {
    { 15, 12 },
    { 11, 14 },
    { 15,  8 },
    { 19, 14 },
...
{
    static const u8 sStoneMaxStepCounts[GESOMON_ROCK_LEVELS - 1] = { 4, 8, 8, 8, 4, 4, 4, 6, 3, 3 };

    if (FlagGet(FLAG_GESOMON_ROCK_COMPLETE) == TRUE)
    {
        gSpecialVar_Result = GESOMON_ROCK_COMPLETE;
        ScriptContext_Enable();
        DestroyTask(taskId);
    }
    else
    {
        ...
        if (rockLevel != 0 && sStoneMaxStepCounts[rockLevel - 1] < stepCount)
        {
            ...
            gSpecialVar_Result = GESOMON_ROCK_FAILED;
            DestroyTask(taskId);
        }
        else if (rockLevel == GESOMON_ROCK_LEVELS - 1)
        {
            FlagSet(FLAG_GESOMON_ROCK_COMPLETE);
            gSpecialVar_Result = GESOMON_ROCK_SOLVED;
            ScriptContext_Enable();
            DestroyTask(taskId);
        }
        else
        {
            ...
            gSpecialVar_Result = GESOMON_ROCK_PROGRESSED;
            DestroyTask(taskId);
        }
    }
}
...
static void ChangeGesomonRockLevel(u8 rockLevel)
{
    ...
    LoadPalette(&sGesomonRockPalettes[rockLevel], OBJ_PLTT_ID(paletteNum), PLTT_SIZEOF(4));
...
void IncrementBirthIslandRockStepCount(void)
{
    ...
    if (gSaveBlock1Ptr->location.mapNum == MAP_NUM(MAP_BIRTH_ISLAND_EXTERIOR) && gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(MAP_BIRTH_ISLAND_EXTERIOR))
    {
        if (++stepCount > 99)
            ...
            VarSet(VAR_GESOMON_ROCK_STEP_COUNT, stepCount);
    }
}
...
void SetGesomonRockPalette(void)
{
    ...
    LoadPalette(&sGesomonRockPalettes[(u8)VarGet(VAR_GESOMON_ROCK_LEVEL)], OBJ_PLTT_ID(paletteNum), PLTT_SIZEOF(4));
...
void CreateAbnormalWeatherEvent(void)
{
    ...
    if (FlagGet(FLAG_DEFEATED_GATOMON_X) == TRUE)
    {
