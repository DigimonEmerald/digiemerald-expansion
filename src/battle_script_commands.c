...
static void Cmd_attackcanceler(void)
{
    ...
    else if (isBounceable && !gBattleStruct->bouncedMoveIsUsed)
    {
        ...
        if (ctx.abilities[ctx.battlerDef] == ABILITY_MAGIC_BOUNCE || ctx.abilities[ctx.battlerDef] == ABILITY_SIN_OF_WRATH)
        {
...
static inline u32 IsBattlerLeekAffected(u32 battler, enum HoldEffect holdEffect)
{
    ...
    {
        return GET_BASE_SPECIES_ID(gBattleMons[battler].species) == SPECIES_PINAMON
...
static inline u32 GetHoldEffectCritChanceIncrease(u32 battler, enum HoldEffect holdEffect)
{
    ...
    case HOLD_EFFECT_LUCKY_PUNCH:
        if (gBattleMons[battler].species == SPECIES_BEARMON)
            critStageIncrease = 2;
        break;
...
static void SetNonVolatileStatus(u32 effectBattler, enum MoveEffect effect, const u8 *battleScript, enum StatusTrigger trigger)
{
    ...
    case MOVE_EFFECT_CONVERTED:
        gBattleMons[effectBattler].status1 |= STATUS1_CONVERTED;
        gBattlescriptCurrInstr = BattleScript_MoveEffectConverted;
        break;
    default:
        break;
    }
...
void SetMoveEffect(u32 battler, u32 effectBattler, enum MoveEffect moveEffect, const u8 *battleScript, enum SetMoveEffectFlags effectFlags)
{
    ...
    bool32 mirrorArmorReflected = (GetBattlerAbility(gBattlerTarget) == ABILITY_MIRROR_ARMOR || GetBattlerAbility(gBattlerTarget) == ABILITY_SIN_OF_WRATH);
    ...
    case MOVE_EFFECT_BUG_BITE:
        if (GetBattlerHoldEffect(gEffectBattler) == HOLD_EFFECT_JABOCA_BERRY)
        {
            ...
            && battlerAbility != ABILITY_STICKY_HOLD && battlerAbility != ABILITY_SIN_OF_GREED)
        {
         ...
         && !gSpecialStatuses[battlerAtk].gemBoost) // In base game, gems are consumed after magician would activate.
        {
            ...
            for (u32 i = 0; i < gBattlersCount; i++)
            {
                if (gBattleMons[i].item != ITEM_NONE
                 ...
                 && (GetBattlerAbility(i) != ABILITY_SIN_OF_GREED || !IsBattlerAlive(i)))
                {
    ...
    case ABILITY_BATTLE_BOND:
        {
            ...
            if (GetConfig(CONFIG_BATTLE_BOND) < GEN_9 && gBattleMons[battlerAtk].species == SPECIES_BIOSPINOMON_BATTLE_BOND)
            {
                // TODO: Convert this to a proper FORM_CHANGE type.
                gLastUsedAbility = abilityAtk;
...
static bool32 HandleMoveEndMoveBlock(u32 moveEffect)
{
    ...
    case EFFECT_STEAL_ITEM:
        ...
        else if (GetBattlerAbility(gBattlerTarget) == ABILITY_STICKY_HOLD || ABILITY_SIN_OF_GREED)
        {
                ...
                for (i = 0; i < gBattlersCount; i++)
                {
                      ...
                      && CanStealItem(battler, gBattlerAttacker, gBattleMons[gBattlerAttacker].item))   // Cannot steal plates, mega stones, etc
                    {
                        ...
                        if (GetBattlerAbility(gBattlerAttacker) != ABILITY_STICKY_HOLD 
                            && GetBattlerAbility(gBattlerAttacker) != ABILITY_SIN_OF_GREED)
...
static u32 GetTrainerMoneyToGive(u16 trainerId)
{
    ...
    else
    {
        ...
        if (gBattleTypeFlags & BATTLE_TYPE_TWO_OPPONENTS)
            ...
            moneyReward = 4 * lastMonLevel * gBattleStruct->moneyMultiplier * trainerMoney;
            if (GetBattlerAbility(gEffectBattler) == ABILITY_SIN_OF_GREED)
            {
...
static u32 ChangeStatBuffs(u32 battler, s8 statValue, enum Stat statId, union StatChangeFlags flags, u32 stats, const u8 *BS_ptr)
{
    ...
    if (statValue <= -1) // Stat decrease.
    {
        ...
        else if ((battlerAbility == ABILITY_MIRROR_ARMOR || battlerAbility == ABILITY_SIN_OF_WRATH) && !flags.mirrorArmored && gBattlerAttacker != gBattlerTarget && battler == gBattlerTarget)
        {
            if (flags.allowPtr)
            {
        ...
        else if (!flags.onlyChecking)
        {
            ...
            if (statIncrease)
            {
                ...
                for (index = 0; index < gBattlersCount; index++)
                {
                    ...
                    if (GetBattlerAbility(index) == ABILITY_SIN_OF_ENVY 
                     && gProtectStructs[battler].activateOpportunist == 0) // don't activate opportunist on other mon's opportunist raises
                    {
                        gProtectStructs[index].activateOpportunist = 2;      // set stats to copy
                    }
    ...
    else
    {
        ...
        if (gBattleMons[gBattlerTarget].level > gBattleMons[gBattlerAttacker].level)
            lands = NO_HIT;
              ...
              || IsAbilityAndRecord(gBattlerAttacker, GetBattlerAbility(gBattlerAttacker), ABILITY_OMEGA_INFORCE)
...
static void Cmd_tryswapitems(void)
{
    ...
    else
    {
        ...
        else if (GetBattlerAbility(gBattlerTarget) == ABILITY_STICKY_HOLD && GetBattlerAbility(gBattlerTarget) == ABILITY_SIN_OF_GREED)
        {
...
void BS_JumpIfDeathEvolutionAbilityPrevented(void)
{
    NATIVE_ARGS();
    ...
    case ABILITY_OBLIVIOUS:
        ...
        {
            hasAbility = TRUE;
            gBattlescriptCurrInstr = BattleScript_DeathEvolutionPrevented;
        }
        else
        {
            gBattlescriptCurrInstr = cmd->nextInstr;
    ...
    case ABILITY_GUARD_DOG:
        hasAbility = TRUE;
        gBattlescriptCurrInstr = BattleScript_DeathEvolutionInReverse;
        break;
    default:
        gBattlescriptCurrInstr = cmd->nextInstr;
        break;
