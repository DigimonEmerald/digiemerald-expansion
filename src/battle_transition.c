...
static void Task_Gargoylemon(u8);
...
static bool8 Gargoylemon_SetGfx(struct Task *);
static bool8 Garurumon_SetGfx(struct Task *);
...
static const u16 sGargoylemon_Palette[] = INCBIN_U16("graphics/battle_transitions/gargoylemon.gbapal");
static const u16 sGarurumon_Palette[] = INCBIN_U16("graphics/battle_transitions/garurumon.gbapal");
static const u16 sGargomon_Palette[] = INCBIN_U16("graphics/battle_transitions/gargomon.gbapal");
static const u32 sGargoylemon_Tilemap[] = INCBIN_U32("graphics/battle_transitions/gargoylemon.bin");
...
static const u32 sGatomon_x_Tileset[] = INCBIN_U32("graphics/battle_transitions/gatomon_x.4bpp.smol");
static const u32 sGatomon_x_Tilemap[] = INCBIN_U32("graphics/battle_transitions/gatomon_x.bin.smolTM");
static const u32 sGekomon_Tileset[] = INCBIN_U32("graphics/battle_transitions/gekomon.4bpp.smol");
static const u32 sGekomon_Tilemap[] = INCBIN_U32("graphics/battle_transitions/gekomon.bin.smolTM");
...
static const TaskFunc sTasks_Main[B_TRANSITION_COUNT] =
{
    ...
    [B_TRANSITION_GARGOYLEMON] = Task_Gargoylemon,
...
static const TransitionStateFunc sGargoylemon_Funcs[] =
{
    Regi_Init,
    Gargoylemon_SetGfx,
    PatternWeave_Blend1,
...
static const TransitionStateFunc sGarurumon_Funcs[] =
{
    Regi_Init,
    Garurumon_SetGfx,
    PatternWeave_Blend1,
...
// B_TRANSITION_GARGOYLEMON, B_TRANSITION_GARURUMON, B_TRANSITION_GARGOMON
...
static void Task_Gargoylemon(u8 taskId)
{
    while (sGargoylemon_Funcs[gTasks[taskId].tState](&gTasks[taskId]));
}

static void Task_Garurumon(u8 taskId)
{
    while (sGarurumon_Funcs[gTasks[taskId].tState](&gTasks[taskId]));
}
...
static bool8 Gargoylemon_SetGfx(struct Task *task)
{
    u16 *tilemap, *tileset;

    GetBg0TilesDst(&tilemap, &tileset);
    LoadPalette(sGargoylemon_Palette, BG_PLTT_ID(15), sizeof(sGargoylemon_Palette));
    CpuCopy16(sGargoylemon_Tilemap, tilemap, 0x500);
    SetSinWave((s16*)gScanlineEffectRegBuffers[0], 0, task->tSinIndex, 132, task->tAmplitude, DISPLAY_HEIGHT);
...
static bool8 Garurumon_SetGfx(struct Task *task)
{
    u16 *tilemap, *tileset;

    GetBg0TilesDst(&tilemap, &tileset);
    LoadPalette(sGarurumon_Palette, BG_PLTT_ID(15), sizeof(sGarurumon_Palette));
    CpuCopy16(sGarurumon_Tilemap, tilemap, 0x500);
    SetSinWave((s16*)gScanlineEffectRegBuffers[0], 0, task->tSinIndex, 132, task->tAmplitude, DISPLAY_HEIGHT);
...
static bool8 Gargomon_SetGfx(struct Task *task)
{
    ...
    GetBg0TilesDst(&tilemap, &tileset);
    LoadPalette(sGargomon_Palette, BG_PLTT_ID(15), sizeof(sGargomon_Palette));
...
static bool8 Gatomon_x_Init(struct Task *task)
{
    ...
    GetBg0TilesDst(&tilemap, &tileset);
    CpuFill16(0, tilemap, BG_SCREEN_SIZE);
    DecompressDataWithHeaderVram(sGatomon_x_Tileset, tileset);
    DecompressDataWithHeaderVram(sGatomon_x_Tilemap, tilemap);
...
static bool8 Gatomon_x_PaletteFlash(struct Task *task)
{
    if (task->tTimer % 3 == 0)
    {
        ...
        offset /= 3;
        LoadPalette(&sGatomon_x1_Palette[offset * 16], BG_PLTT_ID(15), PLTT_SIZE_4BPP);
    }
    if (++task->tTimer > 58)
    {
        task->tState++;
...
static bool8 Gatomon_x_PaletteBrighten(struct Task *task)
{
    if (task->tTimer % 5 == 0)
    {
        ...
        LoadPalette(&sGatomon_x2_Palette[offset * 16], BG_PLTT_ID(15), PLTT_SIZE_4BPP);
    }
    if (++task->tTimer > 68)
    {
        task->tState++;
...
static bool8 Gekomon_Init(struct Task *task)
{
    ...
    GetBg0TilesDst(&tilemap, &tileset);
    CpuFill16(0, tilemap, BG_SCREEN_SIZE);
    DecompressDataWithHeaderVram(sGekomon_Tileset, tileset);
    DecompressDataWithHeaderVram(sGekomon_Tilemap, tilemap);
...
static bool8 Gekomon_PaletteFlash(struct Task *task)
{
    if (task->tTimer % 3 == 0)
    {
        ...
        LoadPalette(&sGekomon1_Palette[offset * 16], BG_PLTT_ID(15), PLTT_SIZE_4BPP);
    }
    if (++task->tTimer > 58)
    {
        task->tState++;
...
static bool8 Gekomon_PaletteBrighten(struct Task *task)
{
    ...
    {
        s16 offset = task->tTimer / 5;
        LoadPalette(&sGekomon2_Palette[offset * 16], BG_PLTT_ID(15), PLTT_SIZE_4BPP);
    }
    if (++task->tTimer > 68)
    {
        task->tState++;
...
static bool8 Geogreymon_Init(struct Task *task)
{
    ...
    task->tState++;
    LoadPalette(&sGeogreymon_Palette[80], BG_PLTT_ID(15), PLTT_SIZE_4BPP);
