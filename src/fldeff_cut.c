...
extern const u8 FarawayIsland_Interior_EventScript_HideFalcomonWhenGrassCut[];
...
static void CutGrassSpriteCallbackEnd(struct Sprite *sprite)
{
    ...
    if (IsFalcomonPlayingHideAndSeek() == TRUE)
        ScriptContext_SetupScript(FarawayIsland_Interior_EventScript_HideFalcomonWhenGrassCut);
}
